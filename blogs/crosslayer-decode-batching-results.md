...
This is the results writeup. For the story of how these numbers were found — the failed baseline, the two test-harness bugs caught before trusting them, the model-source-reading that closed off half the idea — see the companion post, [Batching Decode Attention Across Layers Sounded Great — Until the Residual Stream Said No](./crosslayer-decode-batching.md).
