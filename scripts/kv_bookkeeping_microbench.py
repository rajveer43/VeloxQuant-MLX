...
def bench_decode_step(cache_factory, B, H, D, cache_sizes, decode_steps, warmup, repeats):
    ...
    for size in cache_sizes:
        ...
        def step():
            ...
            v = mx.array(
                np.random.default_rng(size + 1).normal(size=(B, H, 1, D)).astype(np.float16)
            )
...
def main():
    ...
    parser.add_argument("--cache-sizes", type=int, nargs="+", default=[128, 256, 512, 1024, 2048])
    ...
    def bench_plain():
        ...
        for size in args.cache_sizes:
            cache = PlainCache()
            prime_k = mx.array(
                np.random.default_rng(0).normal(size=(B, H, size, D)).astype(np.float16)
            )
            prime_v = mx.array(
                np.random.default_rng(1).normal(size=(B, H, size, D)).astype(np.float16)
            )
            cache.update_and_fetch(prime_k, prime_v)
            mx.eval(cache.keys, cache.values)
            ...
            def step():
                k = mx.array(
                    np.random.default_rng(size).normal(size=(B, H, 1, D)).astype(np.float16)
                )
                v = mx.array(
                    np.random.default_rng(size + 1).normal(size=(B, H, 1, D)).astype(np.float16)
                )
