...
description: Compares VeloxQuant-MLX's 43 selectable KV cache compression methods against llama.cpp's fixed q4_0/q8_0 quantization and plain mlx_lm's uncompressed fp16 cache, with measured compression and speedup numbers.
...
| Compression scheme | None | One uniform per-tensor quant type, same scheme for every layer | 43 methods — VQ, RVQ, non-uniform, low-rank, entropy coding, mixed-precision |
...
- [Algorithm overview](../algorithms/overview) — all 43 methods
