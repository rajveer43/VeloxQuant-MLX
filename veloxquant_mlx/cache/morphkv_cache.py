...
class MorphKVKVCache(_MLXKVCache):
    ...
    # `mlx_lm.models.cache.BatchKVCache`, oblivious to the recent-window
    # correlation eviction state this class needs. Left inherited, every
    ...
    # growth, while the server believes it is still running `morphkv`. This
    ...
    # makes `is_batchable` correctly report `False`, routing `morphkv`
    # through `mlx_lm.server`'s sequential `_serve_single` path instead,
    # where this class already runs correctly. See VeloxQuant-MLX#358 for
    # the full 37-method scope of this defect.
    merge = property(
        lambda self: (_ for _ in ()).throw(
            AttributeError(
                "MorphKVKVCache does not support batched merging; use it via "
