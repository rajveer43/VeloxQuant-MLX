// scalar_affine_attend_batched.metal
// Cross-layer batched sibling of scalar_affine_attend.metal (issue #307 part 1).
// Identical per-(layer,batch,kv-head,query-position) math to
// scalar_affine_attend.metal — the only difference is a new outermost
// NL (num_layers) grid axis, added so one dispatch covers every
// transformer layer's independent decode-attend call instead of one
// dispatch per layer. See _scalar_attend.py's
// scalar_fused_decode_attend_batched docstring and
// docs/KV_KERNEL_ROOFLINE_FINDINGS.md's third addendum for the
// occupancy rationale. Read as plain text at import time via
// _read_kernel_source() and JIT-compiled by mx.fast.metal_kernel at
// call time; it is NOT separately compiled (see issue #64).

    uint tg   = threadgroup_position_in_grid.x;
    uint lane = thread_position_in_threadgroup.x;
    uint sg   = thread_position_in_threadgroup.y;

    uint NL   = uint(q_shape[0]);
    uint B    = uint(q_shape[1]);
    uint H_q  = uint(q_shape[2]);
    uint S_q  = uint(q_shape[3]);
    uint D    = uint(q_shape[4]);
    uint H_kv = uint(k_codes_shape[2]);
    uint S_kv = uint(k_codes_shape[3]);
    uint GK   = uint(k_scale_shape[3]);   // key groups along tokens
    uint GV   = uint(v_scale_shape[4]);   // value groups along channels
    (void)NL;
    ...
    // Each threadgroup owns one (layer, batch, kv-head, query-position) slot
    // and handles ALL HEADS_PER_KV_C query heads that share this kv head —
    // K/V codes are decoded once per (sk, d) and reused across those heads.
    // Layer index is the new outermost axis, peeled off after the same
    // sq/hkv/b derivation scalar_affine_attend.metal already uses.
    uint sq_idx  = tg % S_q;
    uint hkv_idx = (tg / S_q) % H_kv;
    uint b_idx   = (tg / (S_q * H_kv)) % B;
    uint l_idx   = tg / (S_q * H_kv * B);

    // indexes k/v arrays (H_kv-wide, per layer)
    uint bh_kv = (l_idx * B + b_idx) * H_kv + hkv_idx;
    ...
    for (uint sk = sg; sk < S_kv; sk += NSG) {
        ...
        for (uint d = lane; d < D; d += 32u) {
            ...
            for (uint hp = 0; hp < HPK; ++hp) {
                ...
                uint q_off   = ((((l_idx * B + b_idx) * H_q) + h_q_idx) * S_q + sq_idx) * D + d;
    ...
    if (sg == 0) {
        for (uint hp = 0; hp < HPK; ++hp) {
            ...
            for (uint i = 0; i < n_owned; ++i) {
                ...
                uint d = lane + i * 32u;
                if (d < D) {
                    uint out_off = ((((l_idx * B + b_idx) * H_q) + h_q_idx) * S_q + sq_idx) * D + d;
